...
void CReportCtrl::OnCustomDraw(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  if (lplvcd->nmcd.dwDrawStage == CDDS_PREPAINT) {
    ...
    *pResult = CDRF_NOTIFYSUBITEMDRAW;
    ...
    CItemData *p =
        (CItemData *)(CListCtrl::GetItemData(lplvcd->nmcd.dwItemSpec));
    ASSERT(p != NULL);
...
BOOL CReportCtrl::StartEdit(int nItem, int nSubItem) {
  ...
  if (FMT != m_dwPrevEditFmt) {
    ...
    if (!m_pWndEdit->Create(ES_AUTOHSCROLL | ES_NOHIDESEL | WS_CHILD |
                                WS_BORDER | FMT,
                            CRect(0, 0, 1, 1), this, 0))
      return FALSE;
  } else {
    if (m_pWndEdit->GetSafeHwnd() == NULL &&
        !m_pWndEdit->Create(ES_AUTOHSCROLL | ES_NOHIDESEL | WS_CHILD |
                                WS_BORDER | FMT,
                            CRect(0, 0, 1, 1), this, 0)) {
      return FALSE;
    }
